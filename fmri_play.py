...
from fmri_gym import Audio, Display, Session, Triggers, bids
from fmri_gym.config import EXIT_QUIT, load_config, validate_config
from fmri_gym.display import list_monitors, monitor_label

# After fmri_gym, which on import hides the banner pygame would print to stdout
# -- where --next-ses answers a session script's SES=$(...).
import pygame
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--curriculum", help="config JSON of the run (see README); fmri-edit "
                   "writes one without the JSON")
...
def main() -> None:
    ...
    if not args.curriculum:
        p.error("--curriculum is required: the run to play")
